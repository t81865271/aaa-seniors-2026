...
async function loadLogos() {
  try {
    ...
    if (current) updateMajorUndecidedButton();
  } catch (err) {
...
[editNameAr, editNameEn, editMajor, editUniversities].forEach(el => {
  ...
  el.addEventListener("blur", () => {
    if (el === editNameEn) {
      editNameEn.value = titleCaseName(editNameEn.value);
    }

    if (el === editMajor) {
      editMajor.value = editMajor.value.trim() ? titleCaseText(editMajor.value) : " ";
    }
...
function renderLogoLibrary() {
  ...
  logoLibrary.querySelectorAll(".logo-choice").forEach(el => {
    el.addEventListener("click", () => {
      ...
      if (selectedLogoIds.has(id)) {
        selectedLogoIds.delete(id);
      } else {
...
useMajorUndecidedLogoBtn.addEventListener("click", async () => {
  ...
  if (current.major_logo_id === undecidedLogo.id) {
    ...
    // Clear whatever was typed in Major(s).
    editMajor.value = " ";
    current.major = " ";
  }
